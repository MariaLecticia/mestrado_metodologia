...
sample(distNormalSimulacao, 30, replace = FALSE) # se você não tiver o objeto distNormalSimulacao no seu ambiente, crie com o script anterior

# amostragem com reposição usando função sample
sample(distNormalSimulacao, 30, replace = TRUE)

# bootstraping com função replicate
set.seed(450) # agora, não vamos mais usar como tarefa mas como execução ponto a ponto
...
mediaBootsNormal10 <- replicate(30, mean(sample(distNormalSimulacao, 30, replace = TRUE))) # calculamos a média de 10 amostras de 10 casos
mediaBootsNormal50 <- replicate(50, mean(sample(distNormalSimulacao, 30, replace = TRUE))) # calculamos a média de 50 amostras de 10 casos
mediaBootsNormal100 <- replicate(100, mean(sample(distNormalSimulacao, 30, replace = TRUE))) # calculamos a média de 100 amostras de 10 casos
